...
local function injectJass(w2l, buf)
    ...
    -- local start, finish = buf:find('call InitGlobals()', 1, true)
    local bufs = {}
    ...
    local start1,finish1 = buf:find('call InitGlobals()', 1, true)

    bufs[#bufs+1] = buf:sub(finish+1, finish1+1)
    bufs[#bufs+1] = '\r\ncall ExecuteFunc("War3LuaMapPlugin")'
    bufs[#bufs+1] = buf:sub(finish1+1)
