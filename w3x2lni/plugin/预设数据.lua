...
local function insertAbility(w2l)
    ...
    for y = 2, 0,-1 do
        for x = 0, 3 do
            ...
            for _ = 1, SIZEshop do
                i1 = i1 + 1
                local order = orderList[slot1]
                newObject(w2l.slk.ability, nextId(), 'ANcl', 2) {
                    Name = '@商店物品-' .. tostring(slot1),
                    Buttonpos_1 = x,
                    Buttonpos_2 = y,
                    UnButtonpos_1 = x,
                    UnButtonpos_2 = y,
...
local function insertItem(w2l)
    ...
    for i = 1, 360 do
        ...
        newObject(w2l.slk.item, itemNextId(), 'ches') {
...
function mt:on_full(w2l)
    ...
    if w2l.setting.mode == 'lni' or w2l.setting.mode == 'obj' then
        return
    end
