...
local function read_value(self, skill, key)
	...
	if tp == 'table' and not value.type then
		...
		elseif value[self.level] == nil then
			value = value[1]
		else
...
function mt:get_hotkey()
	return self.data.hotkey or self:get_slk('Hotkey', '')
end

--设置技能的热键
--	热键
function mt:set_hotkey()
	local key = self.data.hotkey
	if not key then
		return false
	end
...
--获得技能模板(war3中的技能id)
function mt:get_ability_id(type,slotid,passive)
	local slotid = slotid or self.slotid
	if type == "隐藏" or type == "通用" then
		return nil
	end
	-- if slotid then
	...
	-- end
	if passive then
		return ac.pop(ac.skill.skillList[ac.skill.slotxy[slotid].y][ac.skill.slotxy[slotid].x]["被动"])
	else
		return ac.pop(ac.skill.skillList[ac.skill.slotxy[slotid].y][ac.skill.slotxy[slotid].x]["主动"])
	end
end
...
function unit.__index:add_skill(name, type, slotid, data)
	...
	if not slotid then
		if ac.skill[name].slotid then
			slotid = ac.skill[name].slotid
		else
	...
	skill.ability_id = skill.ability_id or skill:get_ability_id(type, slotid,skill.passive)
	...
	skill:fresh()
	--触发物品自身的添加事件
	local onadd = skill.on_add or skill.data.on_add
	if onadd then
		onadd(skill,skill.owner)
	end
	if type == "物品" then
		if name:find("传送") then
			...
			-- return
		elseif skill then
			skill.create_time = ac.clock()
			ac.item.on_add(skill)
		end
	end
	return skill
...
local function init()
	...
	ac.skill.skillList = {}
	local nextId = ac.nextId('$')
    -- 为每个格子分配100个技能，共1200个
    local SIZE = 100
    local i = 0
    local slot = 0
	for y = 3, 1,-1 do
		ac.skill.skillList[y] = {}
		for x = 1, 4 do
			ac.skill.skillList[y][x] = {}
			ac.skill.skillList[y][x]["主动"] = {}
			ac.skill.skillList[y][x]["被动"] = {}
            slot = slot + 1
            for _ = 1, SIZE do
				i = i + 1
				table_insert(ac.skill.skillList[y][x]["主动"],nextId())
				table_insert(ac.skill.skillList[y][x]["被动"],nextId())
            end
        end
	end
	--记录技能槽位
	...
	for y = 3, 1, -1 do
		for x = 1, 4 do
			table_insert(ac.skill.slotxy,{x=x,y=y})
		end
	end
end
