...
--	[原物品]
--	[额外词缀]
 function item.merge_affixs(dest, other_affixs)
	local affixs = {}
	--添加原物品词缀
	if dest then
...
function self.init_skills()
	...
	for y = 3, 1,-1 do
		for x = 1, 4 do
			for i=1,ac.shop.skillCount do
				local mt = ac.skill[('商店技能%d%d-%d'):format(x, y, i)]
				{
					--技能ID
				...
				ac.shop.skilllist[y][x][#ac.shop.skilllist[y][x]] = nil
				...
				function mt:set_show(item, dest, other_affixs)
					...
					-- print(item.name,is_can_buy)
					self:set_art(item:get_art(nil, false))
				end
				...
				function mt:on_cast_shot()
					...
					if type == '$' then
						...
						if not is_can_buy then
							player:sendMsg('|cfff84545' .. error_tip .. '|r')
							return false
						end
