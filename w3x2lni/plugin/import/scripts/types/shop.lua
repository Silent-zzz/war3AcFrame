...
function mt:get_ability_id(x, y)
	-- return ('*0%d%d'):format(x, y)
	return ac.shop.skilllist[y][x][#ac.shop.skilllist[y][x]]
end

--获得商店技能
function mt:getSkill(x, y)
	return self:find_skill(('商店技能%d%d-%d'):format(x, y, self.shopIndex), nil, true)
end
...
function shop.create(player, unit_id, point,face, page)
	-- shop.rect_name = rect_name
	shop.unitCount = shop.unitCount and shop.unitCount+1 or 1
	--为每个玩家创建一个商店
	...
	local u = player:create_unit(unit_id, point, face)
	setmetatable(u, shop)
	shop[i] = u
	player.shop = u
	
	if page then
		u.page_stack = {u:getPage(page)}
	end
	-- u:set_high(10000)
	...
	for x = 1, 4 do
		for y = 1, 3 do
			u:add_skill(('商店技能%d%d-%d'):format(x, y, shop.unitCount), '隐藏')
		end
	end
	u.shopIndex = shop.unitCount
	--为商店添加马甲物品
	-- for x = 1, 6 do
	-- 	-- u:add_skill('商店物品', '物品', x, {default_item_id = ('ID%d%d'):format(i - 1, x - 1)})
	-- 	u:add_skill('商店物品', '物品', x, {default_item_id = ('*0%d%d'):format(i - 1, x - 1)})
	-- end
...
function shop.init()
	ac.shop = {}
	ac.shop.index = 0
	ac.shop.skillCount = 50
	ac.shop.skilllist = {}
	local shopnext = ac.nextId("*")
	for y = 3, 1, -1 do
		ac.shop.skilllist[y] = {}
		for x = 1, 4 do
			ac.shop.skilllist[y][x] = {}
            for _ = 1, ac.shop.skillCount do
				table.insert(ac.shop.skilllist[y][x],shopnext())
            end
        end
	end
	-- for i=1,12 do
		
	-- 	table.insert(ac.shop.skilllist,shopnext())
	-- end
	require 'types.shop.skill'
	require 'types.shop.page'
	require 'types.shop.item'
	require 'types.shop.init'
end
