...
function mt:get_name()
    return self.name or self:get_slk 'Name' or self:get_slk 'Propernames' 
end
...
function mt:bagSize(n)
    if type(n) == 'number' then
        ...
        jass.UnitAddAbility(self.handle, base.string2id('@BAG'))
        local handle = japi.EXGetUnitAbility(self.handle, base.string2id('@BAG'))
...
function unit.registerJassTriggers()
	...
	--单位丢弃物品事件
	local j_trg = war3.CreateTrigger(function()
		if ac.id[jass.GetItemTypeId(jass.GetManipulatedItem())]:sub(1,1) == "#" then
			return
		end
	...
	local j_trg = war3.CreateTrigger(function()
		local it
		local u = ac.unit(jass.GetManipulatingUnit())
		if ac.item.d_item(jass.GetManipulatedItem()) then
			it = ac.item.d_item(jass.GetManipulatedItem())
			u:event_notify('单位-捡起物品', u, it)
			return
		end
		it = ac.item.j_item(jass.GetManipulatedItem())
		if not it then
			return
		end
		it.owner:event_notify('单位-获得物品', it.owner, it)
	end)
	for i = 1, 13 do
...
function unit.init()
	...
	ac.dummy:add_ability 'Aloc'
	ac.dummy:add_ability 'Arav'
	local j_trg = war3.CreateTrigger(function()
