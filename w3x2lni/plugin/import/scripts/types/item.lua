...
function  item.d_item(handle)
	return drop_items[handle]
end
...
function item.get_id(skill)
	...
	return skill.itemclass and ac.item.itemlist[skill.itemclass][(p:get() - 1)*30+skill:get_slotid()] or ac.item.itemlist["可用可卖"][(p:get() - 1)*30+skill:get_slotid()]
end
...
function mt:get_ability_id()
	...
	return slk.item[self.item_id] and slk.item[self.item_id].abilList or "没有预设技能!"
	-- return '%0' .. ac.getNextId(self.item_id:sub(3, 4))
end
...
function mt:dropItem(unit)
	ac.wait(0,function ()
		local x = jass.GetItemX(self.item_handle)
		local y = jass.GetItemY(self.item_handle)

		local dropitem = {}
		dropitem.name = self.name
		dropitem.title = self:get_title()
		item.on_remove(self)
		self:remove()
		local dropid = ac.item.itemlist['drop'][1]
		dropitem.id = dropid
		table.remove(ac.item.itemlist['drop'],1)
		dropitem.handle = jass.CreateItem(ac.id[dropid],x,y)
		dbg.handle_ref(dropitem.handle)
		drop_items[dropitem.handle] = dropitem
		japi.EXSetItemDataString(ac.id[dropid],4,dropitem.name)
		japi.EXSetItemDataString(ac.id[dropid],3,"4asdfgasdfasdfsad")
...
ac.game:event '单位-捡起物品' (function (trg, hero, it)
	table.insert(ac.item.itemlist['drop'],it.id)
	dbg.handle_unref(it.handle)
	drop_items[it.handle] = nil
	jass.RemoveItem(it.handle)
	hero:add_skill(it.name,"物品")
end)
...
function item:on_add()
	...
	-- print(state.mana,resource.add_max_rate)
...
local function init()
	...
	for i=1, 360 do
		table.insert(item.itemlist["可用不卖"],itemnextid())
		table.insert(item.itemlist["不用不卖"],itemnextid())
		table.insert(item.itemlist["不用可卖"],itemnextid())
		table.insert(item.itemlist["可用可卖"],itemnextid())
	end
	local dropid = ac.nextId("#")
	for i=1,1000 do
		table.insert(item.itemlist["drop"],dropid())
	end
	table.sort(item.itemlist["可用不卖"])
	table.sort(item.itemlist["不用不卖"])
	table.sort(item.itemlist["不用可卖"])
	table.sort(item.itemlist["可用可卖"])
	table.sort(item.itemlist.drop)
end
init()
return item
